...
namespace ObjetPoolModule
{
    ...
    public class ObjectPoolManager : Singleton<ObjectPoolManager>
    {
        ...
        public event Action<PoolObjectProperty> onObjectDiscard;
        public event Action<PoolObjectProperty> onObjectSpawn;
                ...
                if (!string.IsNullOrEmpty(keyValuePair.Key.ToString()))
                {
                    ...
                    loadOp.Completed += (asyncOp) =>
                    {
                        if (asyncOp.Status == AsyncOperationStatus.Succeeded)
                        {
                            ...
                            if (!newPoolProperties.targetObject.TryGetComponent<PoolObjectProperty>(out poolObjectProperty))
                                newPoolProperties.targetObject.AddComponent(typeof(PoolObjectProperty));

                            poolObjectProperty.onDiscard += (prop) =>
                            {
                                onObjectDiscard?.Invoke(prop);
                            };

                            poolObjectProperty.onSpawn += (prop) =>
                            {
                                onObjectSpawn?.Invoke(prop);
                            };
                        }
