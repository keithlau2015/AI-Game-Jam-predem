...
namespace BugReportSystem
{
    internal class TrelloReporter : IReporter
    {
        ...
        private async Task<bool> GetBoardInfo()
        {
            ...
            try
            {
                ...
                if (responseMessage.IsSuccessStatusCode)
                {
                    ...
                    cardMaxLimit = json["limits"]["cards"]["openPerList"]["warnAt"].AsInt;
                    boardID = json["id"].Value;
        ...
        public async Task<bool> SendReport(BugReport bugReport)
        {
            ...
            {
                if (json["name"].Value.Equals(DEFAULT_LIST_NAME))
                {
                    listID = json["id"].Value;
                    break;
                }
            ...
            {
                lastCardID = cardsJson.AsArray[cardsJson.AsArray.Count - 1]["id"].Value;
