...
public class FileManager
{
    ...
    private static void Init()
    {
        ...
        filePath[2] = $"{Application.persistentDataPath}/BattleLog/";
    ...
    public static async void SaveFile<T>(T[] objs, FileType fileType, string fileName = "")
    {
        if (!isInit)
            Init();

        await CreateFile<T>(fileType, fileName, objs);
    }

    public static async void SaveFile<T>(T obj, FileType fileType, string fileName)
    {
        if (!isInit)
            Init();
        ...
        T[] objArray = { obj };
        await CreateFile<T>(fileType, fileName, objArray);
    }
    ...
    private static async UniTask<bool> CreateFile<T>(FileType fileType, string fileName, T[] objs)
    {
        if (!isInit)
            Init();
    ...
    public static async UniTask<List<T>> LoadFile<T>(FileType fileType = FileType.Save, string key = "")
    {
        if (!isInit)
            Init();
